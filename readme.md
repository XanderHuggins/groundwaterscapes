...
This repository contains all scripts necessary to preprocess input data and re-run the groundwaterscape derivation method. Some scripts were executed on clusters provided by the Digital Research Alliance of Canada, so this workflow is not set-up to be run on one's local computer. On this basis, this repository is best understood as a "workbook" that demonstrates all steps undertaken to generate study outcomes.

**Workbook script folder structure**: <br/>
- `on_button.R`: calls the here() function and source runs scripts in `\0-steup` and `\00-functions` folders.
...
- `\scripts\2-analysis\a4-performance`: grid cell residuals from groundwaterscape model.
